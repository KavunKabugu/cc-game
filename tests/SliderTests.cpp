...
void TestSnappingAndClamping() {
    ...
    Slider slider(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 1,.y = 1}}, 0.0f, 10.0f, 2.0f, 4.0f);
...
void TestCallback() {
    Slider slider(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 1,.y = 1}}, 0.0f, 10.0f, 1.0f, 5.0f);
...
void TestLocalCoordinatesToValue() {
    ...
    Slider slider(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 1,.y = 1}}, 0.0f, 100.0f, 1.0f, 0.0f);

    // Usable range is between kThumbHalfNorm (0.02f) and 1.0 - kThumbHalfNorm (0.98f).
    // Test mapping.
    // At x = 0.02f, t = 0.0 -> value = 0.0
    slider.OnMouseButtonDown(SDL_BUTTON_LEFT, UnitPoint{.x = 0.02f, .y = 0.5f});
    assert(slider.GetValue() == 0.0f);

    // At x = 0.98f, t = 1.0 -> value = 100.0
    slider.OnMouseButtonDown(SDL_BUTTON_LEFT, UnitPoint{.x = 0.98f, .y = 0.5f});
    assert(slider.GetValue() == 100.0f);

    // Midpoint: x = 0.5f -> t = (0.5 - 0.02) / 0.96 = 0.5 -> value = 50.0
    slider.OnMouseButtonDown(SDL_BUTTON_LEFT, UnitPoint{.x = 0.50f, .y = 0.5f});
...
void TestMouseAndDragEvents() {
    ...
    [[maybe_unused]] const bool clicked = slider.OnMouseButtonDown(SDL_BUTTON_LEFT, UnitPoint{.x = 0.5f, .y = 0.5f});
    assert(clicked);
    assert(slider.GetValue() == 50.0f);

    // Move drag to local x = 0.74f (t = (0.74 - 0.02) / 0.96 = 0.75 -> value = 75)
    slider.OnDragMove(UnitPoint{.x = 0.74f, .y = 0.5f});
    assert(slider.GetValue() == 75.0f);

    // End drag at local x = 0.98f (value = 100)
    slider.OnDragEnd(SDL_BUTTON_LEFT, UnitPoint{.x = 0.98f, .y = 0.5f});
    assert(slider.GetValue() == 100.0f);
}
