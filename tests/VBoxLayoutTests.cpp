...
void TestCalculateTotalSize() {
    Container parent{UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 1,.y = 1}}};

    // VBoxLayout(spacing = 10, padding = 15, fixedItemHeight = 50)
    const VBoxLayout layout(10.0f, 15.0f, 50.0f);

    // Empty container
    [[maybe_unused]] UnitPoint emptySize = layout.CalculateTotalSize(parent);
    assert(emptySize.x == 0.0f && emptySize.y == 0.0f);

    // Add children
    parent.CreateChild<FakeChild>(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 0,.y = 0}});
    parent.CreateChild<FakeChild>(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 0,.y = 0}});
    ...
    // 1 * spacing = 10
    // total = 30 + 100 + 10 = 140
    [[maybe_unused]] UnitPoint size = layout.CalculateTotalSize(parent);
...
void TestApplyLayout() {
    ...
    const auto* c1 = parent.CreateChild<FakeChild>(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 0,.y = 0}});
    const auto* c2 = parent.CreateChild<FakeChild>(UnitBounds{.min = {.x = 0,.y = 0}, .max = {.x = 0,.y = 0}});

    layout.Apply(parent);

    // Total height = 140
    // c1 bounds: yMin = 15 / 140, yMax = (15 + 50) / 140 = 65 / 140
    [[maybe_unused]] UnitBounds b1 = c1->GetBounds();
    ...
    [[maybe_unused]] UnitBounds b2 = c2->GetBounds();
