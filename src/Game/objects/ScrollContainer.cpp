...
ScrollContainer::ScrollContainer(const UnitBounds bounds) : Container(bounds) {
    ...
    m_trackColor = {.r = 60, .g = 60, .b = 70, .a = 200};
    m_thumbColor = {.r = 220, .g = 220, .b = 230, .a = 255};
}
...
void ScrollContainer::Render(SDL_Renderer* renderer, const SDL_FRect& parentRect) {
    ...
    const SDL_Rect clipRect = {
        .x = static_cast<int>(myRect.x),
        .y = static_cast<int>(myRect.y),
        .w = static_cast<int>(myRect.w),
        .h = static_cast<int>(myRect.h)
    };
    ...
    const SDL_FRect contentRect = {
        .x = myRect.x,
        .y = myRect.y - m_scrollOffsetY,
        .w = myRect.w - m_scrollbarWidth, // Leave space for scrollbar
        .h = m_contentHeight
    };
    ...
    for (const auto& child : children) {
        if (auto* d = child->AsDrawable()) {
            ...
            if (const float childH = (max.y - min.y) * m_contentHeight;
                childY + childH < myRect.y || childY > myRect.y + myRect.h)
    ...
    if (m_contentHeight > myRect.h + 0.1f) {
        ...
        const SDL_FRect track = {
            .x = myRect.x + myRect.w - m_scrollbarWidth, .y = myRect.y, .w = m_scrollbarWidth, .h = myRect.h
        };
...
bool ScrollContainer::HandleEvent(const SDL_Event& event, const UnitPoint parentLocalPos, GameObject*& pressedObject,
                                  GameObject*& hoveredObject)
{
    ...
    const UnitPoint contentLocalPos = {
        .x = localPixelsX / (pixelW - m_scrollbarWidth),
        .y = localPixelsY / m_contentHeight
    };
...
SDL_FRect ScrollContainer::GetThumbRect() const {
    ...
    if (m_contentHeight <= h) return {.x = 0, .y = 0, .w = 0, .h = 0};
    ...
    return {.x = 0, .y = thumbY, .w = m_scrollbarWidth, .h = thumbHeight};
}
