...
namespace
{
    ...
    void LogPath(const std::string_view prefix, const fs::path& path) {
    ...
    std::string MakeSafeFilename(const std::string& name) {
        ...
        std::erase_if(safe, [](const char c) {
    ...
    bool ExtractZip(const fs::path& zipPath, const fs::path& destDir) {
        ...
        FILE* pFile = nullptr;
#ifdef _WIN32
        _wfopen_s(&pFile, zipPath.c_str(), L"rb");
#else
    ...
    std::optional<OsuChart> ParseOsu(const fs::path& filePath) {
        ...
        int colCount = 4;
        if (difficulty.contains("CircleSize")) {
            try {
        ...
        for (const auto& tpLine : timingPointsLines) {
            ...
            try {
                ...
                if (uninherited && beatLength > 0.0) {
                    ...
                    chart.timingPoints.push_back({.timeMs = timeMs, .bpm = std::round(bpm * 100.0) / 100.0});
                }
            } catch (...) {}
        }
        ...
        for (const auto& hoLine : hitObjectsLines) {
            ...
            try {
                ...
                if (objType & 128) {
                    note.type = "hold";
                    if (parts.size() >= 6) {
                        if (auto endParts = Split(parts[5], ':'); !endParts.empty()) {
    ...
    void RunConversion(const std::string& inputPath, const std::string& outputPath) {
        try {
            ...
            for (const auto& charts : groups | std::views::values) {
                if (charts.empty()) continue;

                const auto& [title, artist, version, bpm, audioFile, bgFile] = charts[0].metadata;
                std::string folderName;
                if (groups.size() > 1) {
                    folderName = artist + " - " + title + " [" + version + "]";
                } else {
                ...
                for (const auto& [metadata, timingPoints, notes] : charts) {
                    std::string diffName = metadata.version;
                    ...
                    for (const auto& [timeMs, bpm] : timingPoints) {
                    ...
                    for (const auto& [timeMs, lane, type, durationMs] : notes) {
                        json noteJ = {
                            {"timeMs", timeMs},
                            {"lane", lane},
                            {"type", type}
                ...
                for (const auto& asset : assetsToCopy) {
                    ...
                    if (std::error_code existsCheckEc; fs::exists(srcPath, existsCheckEc)) {
        ...
        for (const char c : text) {
            ...
            for (int row = 0; row < 8; ++row) {
                ...
                for (int col = 0; col < 8; ++col) {
                    if ((row_bits >> col) & 1) {
                        SDL_FRect rect{ .x = x + col * scale, .y = y + row * scale, .w = scale, .h = scale };
...
int main(int argc, char* argv[]) {
    ...
    std::vector<Button> buttons = {
        ...
        {
            .rect = { .x = 40.0f, .y = 210.0f, .w = 150.0f, .h = 30.0f }, .label = "Browse...", .onClick = [window]() {
                SDL_ShowOpenFolderDialog(OutputFolderCallback, nullptr, window, nullptr, false);
            } },
        {
            .rect = { .x = 40.0f, .y = 270.0f, .w = 600.0f, .h = 40.0f }, .label = "CONVERT MAP(S)", .onClick = [window]() {
                std::string in, out;
                {
                    std::lock_guard lock(g_state.mutex);
    ...
    while (running) {
        ...
        // Title
        DrawText8x8(renderer, "cc-game Map Converter", 40.0f, 25.0f, 2.0f, { .r = 255, .g = 255, .b = 255, .a = 255 });
        SDL_SetRenderDrawColor(renderer, 70, 70, 95, 255);
        SDL_FRect titleBar{ .x = 40.0f, .y = 50.0f, .w = 600.0f, .h = 2.0f };
        ...
        // Output section
        DrawText8x8(renderer, "OUTPUT FOLDER (saves into sub-folders):", 40.0f, 160.0f, 1.0f, { .r = 150, .g = 150, .b = 180, .a = 255 });
        ...
        DrawText8x8(renderer, displayOut, 40.0f, 185.0f, 1.0f, { .r = 200, .g = 200, .b = 255, .a = 255 });
        ...
        for (const auto& btn : buttons) {
            ...
            if (convertingSnapshot && btn.label == "CONVERT MAP(S)") {
                ...
                DrawText8x8(renderer, "CONVERTING...", textX, textY, 1.0f, { .r = 100, .g = 100, .b = 110, .a = 255 });
                continue;
            }
            ...
            if (convertingSnapshot) {
                ...
                float textX = btn.rect.x + (btn.rect.w - btn.label.length() * 9.0f) / 2.0f;
                float textY = btn.rect.y + (btn.rect.h - 8.0f) / 2.0f;
                DrawText8x8(renderer, btn.label, textX, textY, 1.0f, { .r = 90, .g = 90, .b = 100, .a = 255 });
                continue;
            }
            ...
            float textX = btn.rect.x + (btn.rect.w - btn.label.length() * 9.0f) / 2.0f;
            float textY = btn.rect.y + (btn.rect.h - 8.0f) / 2.0f;
            DrawText8x8(renderer, btn.label, textX, textY, 1.0f, { .r = 255, .g = 255, .b = 255, .a = 255 });
        }

        // Draw Logs Area
        DrawText8x8(renderer, "PROGRESS LOGS:", 40.0f, 330.0f, 1.0f, { .r = 150, .g = 150, .b = 180, .a = 255 });
        SDL_FRect logBox{ .x = 40.0f, .y = 350.0f, .w = 600.0f, .h = 120.0f };
        ...
        for (size_t idx = startIdx; idx < logsSnapshot.size(); ++idx) {
            ...
            DrawText8x8(renderer, logLine, 50.0f, logY, 1.0f, { .r = 160, .g = 220, .b = 160, .a = 255 }); // Retro green logs
            logY += 11.0f;
        }
